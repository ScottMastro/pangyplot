...
def create_link(fromId, toId, nodeLookup):
    link = dict()
...
class SimpleSnpGraph(SimpleGraph):
    ...
    def to_dictionary(self, nodeLookup, allLinks=False):
        ...
        x,y = self.center()
        bubbleNode = create_node(self.bubbleId, x, y)
        bubbleNode["expand_nodes"] = expand_nodes
        bubbleNode["expand_links"] = expand_links

        links = []
        links.append(create_link(self.linkToId, self.bubbleId, nodeLookup))
        links.append(create_link(self.bubbleId, self.linkFromId, nodeLookup))
